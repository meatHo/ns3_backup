...
main(int argc, char* argv[])
{
    ...
    BandwidthPartInfoPtrVector allBwpss;
    CcBwpCreator ccBwpCreators;
    ...
    band = ccBwpCreators.CreateOperationBandContiguousCc(bandConf);
    ...
    allBwpss = CcBwpCreator::GetAllBwps({band});
    ...
    nrHelper->SetGnbBwpManagerAlgorithmAttribute("NGBR_VIDEO_TCP_PREMIUM", UintegerValue(0));
    std::cout << "hello" << std::endl;
    /*
    ...
    OperationBandInfo bandSl = ccBwpCreators.CreateOperationBandContiguousCc(bandConfSl);
    ...
    NetDeviceContainer gnbNetDev =
        nrHelper->InstallGnbDevice(gNbNodes, allBwpss); // 내가 추가한거내가추가한거내가추가한거
    ...
    for (int i = 0; i < 1; i++)
    {
        nrHelper->GetGnbPhy(gnbNetDev.Get(i), 0)->SetAttribute("Numerology", UintegerValue(0));
