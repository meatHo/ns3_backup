...
// 전역에 저장할 변수
static double g_lastRsrp = 0.0;
static double g_lastRsrq = 0.0;

// Map to store the mapping between Cell IDs and gNB devices
std::map<uint16_t, Ptr<NrGnbNetDevice>> cellIdToGnb;

// rsrp rsrq 측정결과 nr-ue-phy에 정의됨
void
UeMeasCallback(uint16_t cellId,
                uint16_t IMSI,
               uint16_t RNTI,
               double RSRP,
               uint8_t BWPId)
...
LogUeFlowInfo(Ptr<Node> ue)
{
    ...
    {
         std::cout << "🔗 연결된 gNb ID: " << ueDev->GetTargetEnb()->GetCellId() << std::endl;

        std::cout << "🔗 연결된 CELL ID: " << ueDev->GetRrc()->GetCellId() << std::endl;
    }
    else
    {
        std::cout << "⚠️ 아직 gNB에 연결되지 않았습니다." << std::endl;
    }
...
{
    // LogComponentEnable("NrA3RsrpHandoverAlgorithm", LOG_LEVEL_FUNCTION);
    // LogComponentEnable("NrHandoverAlgorithm", LOG_LEVEL_FUNCTION);
    ...
    if (ueDev)
    {
        ...
        phy->TraceConnectWithoutContext("ReportRsrp", MakeCallback(&UeMeasCallback));
    }
    ...
    for (int i = 0; i < gNbNum; i++)
    {
        ...
        // 기지국 번호 확인하기 위해 할당한 것
        Ptr<NrGnbNetDevice> ng = gnbNetDev.Get(i)->GetObject<NrGnbNetDevice>();
        cellIdToGnb[ng->GetCellId()] = ng;
    }
    ...
    Simulator::Schedule(Seconds(1.0), &LogUeFlowInfo, ueNodes.Get(0));
